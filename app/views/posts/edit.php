...
<form class="postForm" action="<?= URLROOT ?>/posts/edit/<?= $data['post']->id ?>" method="post">
    ...
    <div class="field">
        ...
        <span class="invalid-feedback">
            <?=isset($data['title']) ? e($data['title'][0]) : ''?>
        </span> 
        <input type="text" 
            name="title"
            id="title"
            value="<?= e(Input::get('title') === '' ? $data['post']->title : Input::get('title')) ?>" 
    ...
    <div class="field">
        ...
        <span class="invalid-feedback">
            <?=isset($data['body']) ? e($data['body'][0]) : ''?>
        </span>
        <textarea name="body"
        class="<?=isset($data['body']) ? 'is-invalid' : ''?>"
        id="body"> <?= e(Input::get('body') === '' ? $data['post']->body : Input::get('body')) ?> </textarea>
    </div>
